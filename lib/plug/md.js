...
/* spawnHandler takes a directory and the path to a template file */

mdserve.spawnHandler=function(dir){
            ...
            default:
              try{
                fs.readFile(dir+'template.html','utf8',function(err,str){
                  ...
                  res.send(morf.swap(str,{"{TITLE}":"TransitionTech::"+req.url.slice(1),"{CONTENT}":md(markdown)}));
                });
              }catch(err){
                console.log(err);
                next();
